...
type s전체_포트폴리오 struct {
	잠금 sync.RWMutex	// V포트폴리오_구성요소 생성할 때 필요함.
	
	//전체_저장소 map[string]V포트폴리오_구성요소
	전체_저장소 *s고성능_맵

	종목별_저장소 map[string](map[string]V포트폴리오_구성요소)	// 키1 : 종목, 키2 : 전략코드
	전략별_저장소 map[string](map[string]V포트폴리오_구성요소)	// 키1 : 전략코드, 키2 : 종목
...
func (s *s전체_포트폴리오) G구성요소(키 string) V포트폴리오_구성요소 {
	...
	//return v
	return v.(V포트폴리오_구성요소)
}

func (s *s전체_포트폴리오) s추가(전략 C전략, 종목 C종목) (V포트폴리오_구성요소, error) {
	새_구성요소 := NV포트폴리오_구성요소(전략, 종목, 0, 0)
	
	s.잠금.RLock()
	defer s.잠금.RUnlock()
	//_, ok := s.전체_저장소[새_구성요소.G키()]
	_, ok := s.전체_저장소.G값(새_구성요소.G키())
	...
	// 전체 저장소 설정
	//s.전체_저장소[새_구성요소.G키()] = 새_구성요소
	s.전체_저장소.S값(새_구성요소.G키(), 새_구성요소)
	
	F메모("키가 2개인 sharded map을 만들어서 종목별_맵, 전략별_맵에 적용.")
	
	// 종목별 저장소 설정
	종목별_맵, ok := s.종목별_저장소[새_구성요소.G종목().G코드()]
	if !ok {
		종목별_맵 = make(map[string]V포트폴리오_구성요소)
		s.종목별_저장소[새_구성요소.G종목().G코드()] = 종목별_맵
	}
	
	종목별_맵[새_구성요소.G전략().G코드()] = 새_구성요소
	
	// 전략별 저장소 설정
	전략별_맵, ok := s.전략별_저장소[새_구성요소.G전략().G코드()]
	if !ok {
		전략별_맵 = make(map[string]V포트폴리오_구성요소)
		s.전략별_저장소[새_구성요소.G전략().G코드()] = 전략별_맵
	}
	
	전략별_맵[새_구성요소.G종목().G코드()] = 새_구성요소
