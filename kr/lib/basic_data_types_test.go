...
func TestV문자열키_맵(테스트 *testing.T) {
	...
	// 임의값 삽입, 삭제 테스트
	// 총_수량, 동시처리_수량은 go test -race 하에서도 swapping이 발생하지 않도록 조절하였음.
	// 메모리 4GB 64비트 윈도우 8 시스템 기준.
	총_수량 := 150000
	...
	// 처리가 끝날 때까지 대기할 것.
	// 이것도 엄청 중요하더라는.. 
	// 이거 안 하면 데이터가 맵에 채워지기도 전에 수량 비교 테스트가 실행되어서 테스트 실패함.
	for 반복횟수 := 0 ; 반복횟수 < 동시처리_수량 ; 반복횟수++ {
		<-종료_통보_채널
	}
	
	평균_수량 := len(보통_맵) / len(맵.(*sV문자열키_맵).중앙_저장소)
	최소_수량 := int(float64(평균_수량) * 0.8)
	총_수량_확인 := 0
	
	for 키, 맵_조각 := range 맵.(*sV문자열키_맵).중앙_저장소 {		
		맵_조각별_수량 := len(맵_조각.저장소)
		총_수량_확인 = 총_수량_확인 + 맵_조각별_수량
