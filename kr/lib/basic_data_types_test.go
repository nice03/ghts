...
func testV문자열키_맵(테스트 *testing.T, 생성자 func() V문자열키_맵) {
	맵 := 생성자()
	...
	// G값_모음() 테스트
	F같은값_확인(테스트, len(맵.G값_모음()), len(입력값_모음))
	
	for _, 값 := range 맵.G값_모음() {	
		일치 := false
	...
	if _, ok := 맵.(*sV고성능_문자열키_맵); ok {
		평균_수량 := len(보통_맵) / len(맵.(*sV고성능_문자열키_맵).중앙_저장소)
		최소_수량 := int(float64(평균_수량) * 0.5)
	
		for 키, 맵_조각 := range 맵.(*sV고성능_문자열키_맵).중앙_저장소 {		
	...
	// G값_모음() 테스트.
	값_모음 := 맵.G값_모음()
	F같은값_확인(테스트, len(보통_맵), len(값_모음))
	
	for _, 값 := range 값_모음 {
		대조값, 존재함 := 보통_맵[F문자열(값)]
		
		F참인지_확인(테스트, 존재함, "G값_모음() 테스트 실패. %v", 값)
		F같은값_확인(테스트, 값, 대조값)
	}	
...
func TestSV기본_문자열키_맵(테스트 *testing.T) {
	테스트.Parallel()
	...
	testV문자열키_맵(테스트, NV기본_문자열키_맵)	
}

func TestSV고성능_문자열키_맵(테스트 *testing.T) {
	테스트.Parallel()
