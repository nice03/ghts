...
func testI정수(테스트 *testing.T, 생성자 interface{}) {
	...
	case func(int64) V정수:
		...
		i정수 = 생성자_(입력값)
	default:
		panic("testI정수() : 예상치 못한 생성자 형식. " + reflect.TypeOf(생성자).String())
	}
	...
	F다른값_확인(테스트, 출력값, i정수.G값())
	F같은값_확인(테스트, 출력값_백업, i정수.G값())
	F같은값_확인(테스트, 출력값_백업, i정수.G정수())
	F같은값_확인(테스트, 출력값_백업, i정수.G실수())
	F같은값_확인(테스트, 출력값_백업, i정수.G정밀수())
	F같은값_확인(테스트, strconv.FormatInt(출력값_백업, 10), i기본_문자열.String())
...
func TestV통화(테스트 *testing.T) {
	...
	F같은값_확인(테스트, NV통화(통화종류, 100).S나누기(0).G값().GRat(), nil)
...
func TestN정밀수(테스트 *testing.T) {
	...
	bigRat := big.NewRat(100250, 1000)
	도우미_N정밀수(테스트, bigRat, 예상값)
