...
func TestS맵_반환값(테스트 *testing.T) {
	...
	for _, 값 := range 값_모음 {
		정상값 := N맵_반환값(값, true)
		에러값 := N맵_반환값(nil, false)
		
		F같은값_확인(테스트, 정상값.G값(), 값)
		F참인지_확인(테스트, 정상값.G찾았음())
		
		F같은값_확인(테스트, 에러값.G값(), nil)
		F거짓인지_확인(테스트, 에러값.G찾았음())
	}
}
...
func TestI안전한_맵(테스트 *testing.T) {
	...
	F같은값_확인(테스트, 맵1.G값("첫번째").G값(), "1")
	F같은값_확인(테스트, 맵1.G값("첫번째").G찾았음(), true)
	F같은값_확인(테스트, 맵1.G값("두번째").G값(), nil)
	F같은값_확인(테스트, 맵1.G값("두번째").G찾았음(), false)
	
	F같은값_확인(테스트, 맵2_1.G길이(), 2)
	F같은값_확인(테스트, 맵2_1.G값("첫번째").G값(), "1")
	F같은값_확인(테스트, 맵2_1.G값("첫번째").G찾았음(), true)
	F같은값_확인(테스트, 맵2_1.G값("두번째").G값(), "2_1")
	F같은값_확인(테스트, 맵2_1.G값("두번째").G찾았음(), true)
	
	F같은값_확인(테스트, 맵2_2.G길이(), 2)
	F같은값_확인(테스트, 맵2_2.G값("첫번째").G값(), "1")
	F같은값_확인(테스트, 맵2_2.G값("첫번째").G찾았음(), true)
	F같은값_확인(테스트, 맵2_2.G값("두번째").G값(), "2_2")
	F같은값_확인(테스트, 맵2_2.G값("두번째").G찾았음(), true)
	
	// immutable 확인.
	맵1 = 맵1.S값("첫번째", "100")
	
	F같은값_확인(테스트, 맵1.G길이(), 1)
	F같은값_확인(테스트, 맵1.G값("첫번째").G값(), "100")
	...
	F같은값_확인(테스트, 맵1.G값("첫번째").G값(), "100")	
	F같은값_확인(테스트, 맵2_1.G값("첫번째").G값(), "1000_001")	
	F같은값_확인(테스트, 맵2_2.G값("첫번째").G값(), "1")
	
	맵2_2 = 맵2_1.S값("두번째", "200_200")
	F같은값_확인(테스트, 맵2_2.G값("두번째").G값(), "200_200")
    ...
    for 인덱스 := 0 ; 인덱스 < 길이 ; 인덱스++ {
		switch 인덱스 {
		case 7, 19, 42, 99:
			F같은값_확인(테스트, 맵.G값(F문자열(인덱스)).G값(), nil)
			F같은값_확인(테스트, 맵.G값(F문자열(인덱스)).G찾았음(), false)
		default:
			F같은값_확인(테스트, 맵.G값(F문자열(인덱스)).G값(), 인덱스 * 10)
			F같은값_확인(테스트, 맵.G값(F문자열(인덱스)).G찾았음(), true)
		}
    }
