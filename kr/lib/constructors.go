...
func NV기본_문자열키_맵() V문자열키_맵 {
	저장소 := make(map[string]I가변형)
	
	return &sV기본_문자열키_맵{저장소: 저장소}
}

func NV고성능_문자열키_맵() V문자열키_맵 {
	중앙_저장소 := make(map[string]*sV기본_문자열키_맵, 256)

	for 인덱스 := 0; 인덱스 < 256; 인덱스++ {
		키 := fmt.Sprintf("%02x", 인덱스) // 2자리 16진수. 16 * 16 = 256.
		저장소 := make(map[string]I가변형)

		중앙_저장소[키] = &sV기본_문자열키_맵{저장소: 저장소}
	}

	return &sV고성능_문자열키_맵{중앙_저장소: 중앙_저장소}
}
...
func NV포트폴리오_구성요소(종목 C종목, 전략 C전략, 
	...
	return &sV포트폴리오_구성요소{s포트폴리오_구성요소: 내부값}
}
...
func NC전략별_포트폴리오(
		전략 C전략, 
		종목별_포트폴리오_모음 []C종목별_포트폴리오) C전략별_포트폴리오 {
	저장소 := make(map[string]C종목별_포트폴리오)
	
	for _, 종목별_포트폴리오 := range 종목별_포트폴리오_모음 {
		저장소[종목별_포트폴리오.G종목().G코드()] = 종목별_포트폴리오
	}
...
func NV포트폴리오(자본 C통화) V포트폴리오 {
	return &sV포트폴리오{
		자본: 자본.G변수형(), 
		전체_저장소: NV고성능_문자열키_맵(),
		종목별_저장소: NV고성능_문자열키_맵(),
		전략별_저장소: NV고성능_문자열키_맵()}
}