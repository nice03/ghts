...
func 메모() {
	...
	fmt.Println("	  : 포트폴리오는 mutable한 데이터의 대표적인 예임.")
	fmt.Println("		게다가, 동시에 여러 종목의 값이 변하므로, Lock에 의존하면")
	fmt.Println("		데드락이 발생할 가능성이 높음.")
	fmt.Println("		STM이 없는 이상 Go언어의 채널을 통한 동기화가 적절함.")
	fmt.Println("	- 위험관리")
