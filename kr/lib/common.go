...
func 메모() {
	...
	fmt.Println("			   이에 대한 근본적인 해결책은 ")
	fmt.Println("			   - immutable 및 자동복사 되는 데이터만 주고 받기.")
	fmt.Println("			   - 함수나 메소드는 외부 mutable 데이터를 직접 건드리지 않기.")
	fmt.Println("			   - mutable 데이터 변경은 채널을 통해서 동기화 하기")
	fmt.Println("			  이 3가지만 지키면 mutable 데이터는 동시 접근되지 않는다.")
	fmt.Println("			  이 3가지를 자동으로 검사 및 테스트 할 방법은 없을까?")
	fmt.Println("")
