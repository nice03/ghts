...
func TestS가상TB(테스트 *testing.T) {
	가상_테스트 := new(s가상TB)
	var tb testing.TB = 가상_테스트; tb.Failed()
	var i테스트용_가상_객체 I테스트용_가상_객체 = 가상_테스트; i테스트용_가상_객체.테스트용_가상_객체()
...
func TestF참인지_확인(테스트 *testing.T) {
	...
	if !테스트_통과 {
		파일명, 행_번호 := F소스코드_위치(1)
		테스트.Error("%s:%d: 예상치 못한 테스트 실패.", 파일명, 행_번호)
	}

	테스트_통과 = true
	F참인지_확인(가상_테스트, false, "")
	if 테스트_통과 {
		파일명, 행_번호 := F소스코드_위치(1)
		테스트.Error("%s:%d: 예상치 못한 테스트 통과.", 파일명, 행_번호)
	}
}
