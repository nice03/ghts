...
func TestV통화(테스트 *testing.T) {
	...
	F같은값_확인(테스트, v통화.S절대값(c통화1), NC통화(통화종류, 100))
	F같은값_확인(테스트, v통화.S절대값(c통화2), NC통화(통화종류, 100))
	
	F같은값_확인(테스트, v통화.S더하기(c통화1, c통화2), NC통화(통화종류, 0))
	F같은값_확인(테스트, v통화.S빼기(c통화1, c통화2), NC통화(통화종류, 200))
	F같은값_확인(테스트, v통화.S곱하기(c통화1, c통화2), NC통화(통화종류, -10000))
	
	v통화, 에러 := v통화.S나누기(c통화1, c통화2)
	F에러없음_확인(테스트, 에러)
	F같은값_확인(테스트, v통화, NC통화(통화종류, -1))
	
	v통화, 에러 = v통화.S나누기(c통화1, NC통화(통화종류, 0))
	F에러발생_확인(테스트, 에러)
	F참인지_확인(테스트, v통화 == nil)
	
	v통화 = NV통화(통화종류, 0)
	F같은값_확인(테스트, v통화.S반대부호값(c통화1).G값(), -100)
	F같은값_확인(테스트, v통화.S반대부호값(c통화2).G값(), 100)
	
	// 셀프 연산함수 테스트
	F같은값_확인(테스트, NV통화(통화종류, -100).S셀프_절대값().G값(), 100)
	F같은값_확인(테스트, NV통화(통화종류, 100).S셀프_절대값().G값(), 100)
	
	F같은값_확인(테스트, NV통화(통화종류, 100).S셀프_더하기(c통화1).G값(), 200)
	F같은값_확인(테스트, NV통화(통화종류, 100).S셀프_빼기(c통화1).G값(), 0)
	F같은값_확인(테스트, NV통화(통화종류, 100).S셀프_곱하기(c통화1).G값(), 10000)
	
	v통화, 에러 = NV통화(통화종류, 100).S셀프_나누기(NC통화(통화종류, 10))
	F에러없음_확인(테스트, 에러)
	F같은값_확인(테스트, v통화.G값(), 10)
	
	v통화, 에러 = NV통화(통화종류, 100).S셀프_나누기(NC통화(통화종류, 0))
	F에러발생_확인(테스트, 에러)
	F참인지_확인(테스트, v통화 == nil)
	
	F같은값_확인(테스트, NV통화(통화종류, 100).S셀프_반대부호값().G값(), -100)
	F같은값_확인(테스트, NV통화(통화종류, -100).S셀프_반대부호값().G값(), 100)
}